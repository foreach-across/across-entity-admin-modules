...
export function executeRequest(partialConfiguration: RequestConfiguration): Promise<Response> {
    ...
    if (partialConfiguration.additionalQueryParameters) {
        Object.keys(partialConfiguration.additionalQueryParameters).forEach((queryParamName) => {
            const value = partialConfiguration.additionalQueryParameters[queryParamName];

            if (responseUrl && value === UPDATE_ID_VALUE) {
                const id = getIdFromUpdateUrl(responseUrl);

                if (id !== null) {
                    partialConfiguration.additionalQueryParameters[queryParamName] = id;
                }
            }
        });
    }
    ...
    if (copyOriginalRequestParameters) {
        ...
        for (const entry of queryParams.entries()) {
            ...
            if (!partialConfiguration.additionalQueryParameters[name]) {
                partialConfiguration.additionalQueryParameters[name] = value;
            }
        }
    }
    ...
    if (typeof formToSerialize !== "undefined") {
        ...
        return executeFormRequest(
            ...
            formToSerialize,
            requestConfig,
            partialConfiguration.additionalQueryParameters
        );
    }
