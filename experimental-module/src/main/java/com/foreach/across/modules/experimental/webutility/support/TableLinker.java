...
public class TableLinker
{
	...
	public void createLinkOnProperty( SortableTableBuilder sortableTableBuilder, String property, LinkDestination linkDestination ) {
		sortableTableBuilder.valueRowProcessor( ( ctx, row ) ->
				                                        row.getChildren().stream()
				                                           ...
				                                           .findFirst()
				                                           .ifPresent(
						                                           propertyCell -> addLink( getItem( (IteratorViewElementBuilderContext) ctx ), propertyCell,
						                                                                    linkDestination, null ) ) );
	}

	public void createLinkOnProperty( SortableTableBuilder sortableTableBuilder,
	                                  String property,
	...
	private void addLink( Object rowEntity, TableViewElement.Cell propertyCell, LinkDestination linkDestination, EntityViewLinkBuilder linkBuilder ) {
		if ( propertyCell.getChildren().size() == 1 && rowEntity != null ) {
			...
			String url =
					linkDestination == LinkDestination.DETAIL ? forEntityConfiguration.withFromUrl( currentUrl ).toUriString() : forEntityConfiguration
							.updateView().withFromUrl( currentUrl ).toUriString();

			if ( linkBuilder != null ) {
				currentUrl = linkBuilder.toUriString();
				url = linkDestination == LinkDestination.DETAIL ? linkBuilder.withFromUrl( currentUrl ).toUriString() : linkBuilder.forInstance( rowEntity )
				                                                                                                                   .updateView().withFromUrl(
								currentUrl ).toUriString();
			}

			LinkViewElement link = bootstrap.builders.link().url( url )
			                                         .build();
