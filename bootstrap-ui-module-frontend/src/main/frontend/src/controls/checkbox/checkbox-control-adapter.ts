...
export default class CheckboxControlAdapter extends BaseControlAdapter {
  ...
  constructor(target: any) {
    const element: any = isUnwrapped(target)
      ? target
      : $(target).find('input[type=checkbox], input[type=radio]')[0];
    super(element);
    this.initialValue = this.isSelected();
    $(element).change(_ => this.triggerChange());
    ...
    if (this.isSelected()) {
      ...
      return [
        createControlValueHolder(
          labelElement.length > 0 ? labelElement.text() : undefined,
          target.attr('value'),
          this.getTarget()
        ),
      ];
 ...
 */
export function createCheckboxControlAdapter(
  node: any
