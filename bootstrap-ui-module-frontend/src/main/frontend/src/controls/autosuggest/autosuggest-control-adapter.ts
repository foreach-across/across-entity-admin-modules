...
export default class AutosuggestControlAdapter extends BaseControlAdapter {
  ...
  constructor(wrapper: any) {
    ...
    $(typeahead).on('typeahead:change', _ => this.triggerChange());
    $(typeahead).on('typeahead:open', _ => {
      // @ts-ignore
      $(this.valueHolder).val(null);
      this.triggerChange();
    });
    $(typeahead).on('typeahead:select', _ => this.triggerChange());
    ...
    this.initialValue = {
      label: typeahead.typeahead('val'),
      value: typeaheadValue.val(),
    };
  }
  ...
  getValue(): BootstrapUiControlValueHolder[] {
    ...
    return [
      createControlValueHolder(label, value, {
        typeahead: this.getTarget(),
        typeaheadValue: this.valueHolder,
      }),
    ];
  }
 ...
 */
export function createAutosuggestControlAdapter(
  node: any
