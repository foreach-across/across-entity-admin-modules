...
export default {
   ...
   */
  assertHasUnderlyingControlAdapters: function(
    selector,
    amountOfControlAdapters
  ) {
    cy.get(selector).then(item => {
      expect(item.find('[data-bootstrapui-adapter-type]')).to.have.property(
        'length',
        amountOfControlAdapters || 0
      );
    });
  },
  /**
