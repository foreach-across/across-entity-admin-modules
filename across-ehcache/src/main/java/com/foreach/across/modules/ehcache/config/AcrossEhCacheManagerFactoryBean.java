...
public class AcrossEhCacheManagerFactoryBean implements FactoryBean<CacheManager>, InitializingBean, DisposableBean
{
	 ...
	 */
	public void setConfigLocation(Object config) {
		this.config = config;
	}
	...
	public void afterPropertiesSet() throws IOException, CacheException {
		...
		InputStream is = (this.config != null && this.config instanceof Resource) ? ((Resource) this.config).getInputStream() : null;
		try {
			Configuration configuration;
			if( this.config instanceof Resource ) {
				configuration = (is != null ?
						ConfigurationFactory.parseConfiguration(is) : ConfigurationFactory.parseConfiguration());
			} else  if( this.config instanceof Configuration ) {
